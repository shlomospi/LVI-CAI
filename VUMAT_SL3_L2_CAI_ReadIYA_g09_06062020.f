...
c************************************
      dimension jblock(*),props(nprops), density(*),coordMp(*),!(nblock), coordMp(nblock,*),
     1  charLength(*), strainInc(*),!(nblock,ndir+nshr),
     2  relSpinInc(*), tempOld(*),
     3  stretchOld(*),
     4  defgradOld(*),
     5  fieldOld(*), stressOld(*),!(nblock,ndir+nshr),
     6  stateOld(*),enerInternOld(*),!(nblock,nstatev), enerInternOld(*),
     7  enerInelasOld(*), tempNew(*),
     8  stretchNew(*),
     8  defgradNew(*),
     9  fieldNew(*),
     1  stressNew(*),stateNew(*),!(nblock,ndir+nshr), stateNew(nblock,nstatev),
     2  enerInternNew(*), enerInelasNew(*)
c
		COMMON /SHAREDF/ KSDVSa
		  COMMON /SHAREDF/ KSDVSb
		  COMMON /SHAREDF/ KSDVSc
		  COMMON /SHAREDF/ KSDVSd
		  COMMON /SHAREDF/ KSDVSe
		  COMMON /SHAREDF/ KSDVSf
		  COMMON /SHAREDF/ KSDVSg
		  COMMON /SHAREDF/ KSDVSh
		  COMMON /SHAREDF/ KSDVSi
		  COMMON /SHAREDF/ KSDVSj
		  	  	  COMMON /SHAREDF/ KSDVSk
		COMMON /SHAREDF/ NumOfElem
     	Real*8 KSDVSa (20000,47),NumOfElem !<------------ set here
		   Real*8 KSDVSb (20000,47) 
		   Real*8 KSDVSc (20000,47)
		   Real*8 KSDVSd (20000,47)
		   Real*8 KSDVSe (20000,47)
		   Real*8 KSDVSf (20000,47)
		   Real*8 KSDVSg (20000,47)
		   Real*8 KSDVSh (20000,47)
		   Real*8 KSDVSi (20000,47)
		   Real*8 KSDVSj (20000,47)
		   Real*8 KSDVSk (30400,47)
...
c   Local Arrays/scalars Variables
     ...
     &  G_ABED_TEMP(6,6),G_ABED_interface(6,6),G_ABED_SET(6*(N_layer+1),6)
		...
		real*8 TempVec(N_layer),ReadFlag,DamageIndex,ElementNumber,stressPower
		Real*8 VecIndex(46),tempwrite
...
C
c******************************************************************************
C Read the information stored in the common SDVs Matrix which is read in the begining
C***************************************************************************
C		
		 ReadFlag = stateOld(i,9)
		 if (ReadFlag.eq.0) then
			 ElementNumber = nElement(i)
			if (ElementNumber.lt.20000.5) then
			 do j =1,46
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSa(ElementNumber,j+1)
				 StateOld(i,DamageIndex) = KSDVSa(ElementNumber,j+1)
			 end do
			 elseif (ElementNumber.lt.40000.5) then
				 do j =1,46
				 DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSb(ElementNumber-20000,j+1)
				 StateOld(i,DamageIndex) = KSDVSb(ElementNumber-20000,j+1)
			     end do
			 elseif (ElementNumber.lt.60000.5) then
				 do j =1,46
				 DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSc(ElementNumber-40000,j+1)
				 StateOld(i,DamageIndex) = KSDVSc(ElementNumber-40000,j+1)
				 end do
			 elseif (ElementNumber.lt.80000.5) then
			 	do j =1,46		 
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSd(ElementNumber-60000,j+1)
				 StateOld(i,DamageIndex) = KSDVSd(ElementNumber-60000,j+1)
				 end do
			 elseif (ElementNumber.lt.100000.5) then
			 	do j =1,46		 
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSe(ElementNumber-80000,j+1)
				 StateOld(i,DamageIndex) = KSDVSe(ElementNumber-80000,j+1)
				 end do
			 elseif (ElementNumber.lt.120000.5) then
			 	do j =1,46		 
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSf(ElementNumber-100000,j+1)
				 StateOld(i,DamageIndex) = KSDVSf(ElementNumber-100000,j+1)
				 end do
			 elseif (ElementNumber.lt.140000.5) then
			 	do j =1,46		 
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSg(ElementNumber-120000,j+1)
				 StateOld(i,DamageIndex) = KSDVSg(ElementNumber-120000,j+1)
				 end do
			 elseif (ElementNumber.lt.160000.5) then
			 	do j =1,46		 
				DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSh(ElementNumber-140000,j+1)
				 StateOld(i,DamageIndex) = KSDVSh(ElementNumber-140000,j+1)
				 end do
			 elseif (ElementNumber.lt.180000.5) then
			 	do j =1,46		 
				 DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSi(ElementNumber-160000,j+1)
				 StateOld(i,DamageIndex) = KSDVSi(ElementNumber-160000,j+1)
				 end do
			 elseif (ElementNumber.lt.200000.5) then
			 	do j =1,46		 
				 DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSj(ElementNumber-180000,j+1)
				 StateOld(i,DamageIndex) = KSDVSj(ElementNumber-180000,j+1)
				 end do
			 else
			 	do j =1,46
				 DamageIndex = VecIndex(j)
				 StateNew(i,DamageIndex) = KSDVSk(ElementNumber-200000,j+1)
				 StateOld(i,DamageIndex) = KSDVSk(ElementNumber-200000,j+1)
				 end do
			 end if
			 stateNew(i,9) = 1
	     else
...
C
              if (E_IP_L(1).gt.0) then ! fiber tension
					  ...
					  if ((D11.gt.(gamma1t-0.001)).and.(k.eq.2)) then !(theta(k).eq.0)) then ! fiber failed in the layer, reduce the layer to zero
							StateNew(i,10) = 0 
					  ! goto 100
					  ! to ensure that the layer is deleted, we make both fiber damage parameters to be equal the maximal value
						  ! StateOld(i,2+11*k) = gamma1c
						  ! StateNew(i,2+11*k) = gamma1c ! can a damage parameter exist without an I strain?
							! do j = 1,6
								! do i1 = 11,6
									! CG_A_TEMP(j,i1) = 0.0 									
								! end do
					  ...
					  if ((D11.gt.(gamma1t-0.001)).and.(k.eq.2)) then !(theta(k).eq.0)) then ! fiber failed in the layer, reduce the layer to zero
							StateNew(i,10) = 0 
					  ! goto 100
						  ! StateOld(i,1+11*k) = gamma1t
						  ! StateNew(i,1+11*k) = gamma1t ! can a damage parameter exist without an I strain?
							! do j = 1,6
								! do i1 = 11,6
									! CG_A_TEMP(j,i1) = 0.0 									
								! end do
...
C Update the specific internal energy - 
        stressPower = 
     &  0.5 * (( stressOld(i,1)+stressNew(i,1) )*strainInc(i,1)
...
C
	  ...
	  COMMON /SHAREDF/ NumOfElem
	   ...
	   integer NumOfElem!, linenum
       Real*8 KSDVSa (20000,47) !<------------ set here
	   Real*8 KSDVSb (20000,47) !230400
	   Real*8 KSDVSc (20000,47)
	   Real*8 KSDVSd (20000,47)
	   Real*8 KSDVSe (20000,47)
	   Real*8 KSDVSf (20000,47)
...
C     Start of the analysis
      if (lOp .eq. j_int_StartAnalysis) then
		!read file, count num of lines, assign this as the number of elements
	!    open (15, file='D:\Abaqus_projects\AFRL\IMPACT_SHLOMO_APR2020\
    ! & TestingCAIVUMAT\CUBE_CAI_SL_TEST_SDV_GENERATE_SDVs.CSV', status='old')
C		
     	NumOfElem = 230400  !<------------ set here
		!linenum = 0
C  
		
		!Real*8 KSDVS(NumOfElem,27)
		write(*,*) "opening file..."
       open (15, file=
     &'/scratch2/eliasy/impact/Sublaminate_L2_impact_readme_SDVs.csv'
     &, status='old') !<------------ set here
		write(*,*) "opened file"
		!write(*,*) "KSDVS_shape:", Shape(KSDVSa)
        do j= 1,NumOfElem !read the NumOfElem layers and assign the values to the common array KSDVS
			if (j.lt.20000.5) then
		 read(15,*) (KSDVSa(j,ii),ii=1,47)
			else if (j.lt.40000.5) then
		 read(15,*) (KSDVSb(j-20000,ii),ii=1,47)			
			else if (j.lt.60000.5) then
		 read(15,*) (KSDVSc(j-40000,ii),ii=1,47)			
			else if (j.lt.80000.5) then
		 read(15,*) (KSDVSd(j-60000,ii),ii=1,47)			
			else if (j.lt.100000.5) then
		 read(15,*) (KSDVSe(j-80000,ii),ii=1,47)			
			else if (j.lt.120000.5) then
		 read(15,*) (KSDVSf(j-100000,ii),ii=1,47)			
			else if (j.lt.140000.5) then
		 read(15,*) (KSDVSg(j-120000,ii),ii=1,47)			
			else if (j.lt.160000.5) then
		 read(15,*) (KSDVSh(j-140000,ii),ii=1,47)			
			else if (j.lt.180000.5) then
		 read(15,*) (KSDVSi(j-160000,ii),ii=1,47)		
			else if (j.lt.200000.5) then
		 read(15,*) (KSDVSj(j-180000,ii),ii=1,47)			
			else 
		 read(15,*) (KSDVSk(j-200000,ii),ii=1,47)
			end if
		end do
		write(*,*) "finished reading"
		write(*,*) "KSDVS11",KSDVSa(1,1)
				close(15,status='keep')
		write(*,*) "closed the file"
		write(*,*) "KSDVS21",KSDVSa(2,1)
...
C         The initial values may need to match those at the point of restart.
